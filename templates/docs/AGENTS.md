...
3. Check the active feature in `blinder/feature_list.json` (or `blinder.sh next`).
...
| **planner** | main thread | Split an initiative into features (thin); insert via `blinder.sh new`; keep `roadmap.md` | Design or implement; lock per-feature detail |
