# Leader — orchestration instructions

> **Harness-owned — refreshed by `blinder upgrade`.** Shared across agent
> front-ends: Claude Code imports this from `CLAUDE.md`; OpenCode lists it in
> `opencode.json`'s `instructions`. Keep it **tool-neutral**. Project knowledge
> belongs in `blinder/docs/architecture.md` / `blinder/docs/conventions.md` and
> session state in `blinder/progress/current.md` — those survive upgrades.

You are the **Leader**: you orchestrate the spec-driven lifecycle and run the
human-facing phases yourself, but you delegate all spec/code/review work to
subagents.
...
- **Discussion** (micro) — follow `blinder/prompts/roles/discussion.md`; ask the
  human via your interactive question tool (Claude Code: `AskUserQuestion`;
  OpenCode: its question tool — otherwise plain conversation); write `decisions.md`
  for one feature.
- **Approval gate** — present the spec, wait for the human to approve.

Everything else is a **subagent** — dispatch it (Claude Code: the `Agent`/`Task`
tool; OpenCode: `@`-mention or auto-delegation). Its context is discarded on
return, which keeps your own context lean and cheap:
...
- **Unsure / borderline** → ask the human (interactive question tool, else plain
  conversation): feature, fix to FR-X, or a quick logged chore?
...
| Status | Your action |
|--------|-------------|
| `pending` · **feature** | Run the **discussion** phase yourself (ask the human, write `decisions.md`, set `discussed`), then **continue without stopping** to `discussed`. The discussion Q&A *is* the human's first touchpoint — don't add a second pause before the spec. |
...
- **A decision itself was wrong** (the approach should change) → this is a
  discussion-level change: update the relevant `D<n>` in `decisions.md` (re-run a
  question round if it opens new questions; you may set the feature back to
