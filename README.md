...
| `bash blinder/cli.sh log "message"` | Record a **chore** — a change too small for a tracked unit — as a one-line entry in `history.md`. |
...
- **Three lanes** (right-sized process): a **feature** runs the full loop; a **fix**
  (`--type fix --fixes FR-X`) skips discussion and is regression-test-first; a
  **chore** (`cli.sh log`) is a known, non-behavioral edit the Leader makes directly
  and logs — no unit, no cycle.
